// Six original critters in a flat blocky pixel style: front-facing, no
// outlines, big square black eyes, soft muted palette. All share the same
// grammar — chunky silhouette on a ~16×20 grid, wide-set 2×2 eyes, a small
// nose/beak accent, and gently notched corners so nothing reads as a plain box.

import { PixelBuilder, PixelModel } from "./pixelModel";

const INK = "#0a0d0c";

// Hand-tuned in the critter editor (/preview/critter-editor).
const bunny = (): PixelModel => ({
  id: "bunny",
  name: "Biscuit the Bunny",
  description: "A cream bunny with orange-lined ears and a bright pink nose.",
  cells: [
    { x: 6, y: 6, c: "#f6f1da" },
    { x: 6, y: 7, c: "#f6f1da" },
    { x: 6, y: 8, c: "#f6f1da" },
    { x: 6, y: 9, c: "#f6f1da" },
    { x: 6, y: 10, c: "#f6f1da" },
    { x: 7, y: 6, c: "#f6f1da" },
    { x: 7, y: 7, c: "#f4935e" },
    { x: 7, y: 8, c: "#f4935e" },
    { x: 7, y: 9, c: "#f4935e" },
    { x: 7, y: 10, c: "#f6f1da" },
    { x: 8, y: 6, c: "#f6f1da" },
    { x: 8, y: 7, c: "#f6f1da" },
    { x: 8, y: 8, c: "#f6f1da" },
    { x: 8, y: 9, c: "#f6f1da" },
    { x: 8, y: 10, c: "#f6f1da" },
    { x: 13, y: 6, c: "#f4935e" },
    { x: 13, y: 7, c: "#f4935e" },
    { x: 13, y: 8, c: "#f4935e" },
    { x: 13, y: 9, c: "#f4935e" },
    { x: 13, y: 10, c: "#f6f1da" },
    { x: 13, y: 11, c: "#f6f1da" },
    { x: 14, y: 11, c: "#f6f1da" },
    { x: 15, y: 11, c: "#f6f1da" },
    { x: 5, y: 11, c: "#f6f1da" },
    { x: 5, y: 12, c: "#f6f1da" },
    { x: 5, y: 13, c: "#f6f1da" },
    { x: 5, y: 14, c: "#f6f1da" },
    { x: 5, y: 15, c: "#f6f1da" },
    { x: 5, y: 16, c: "#f6f1da" },
    { x: 5, y: 17, c: "#f6f1da" },
    { x: 6, y: 11, c: "#f6f1da" },
    { x: 6, y: 12, c: "#f6f1da" },
    { x: 6, y: 13, c: "#f6f1da" },
    { x: 6, y: 14, c: "#f6f1da" },
    { x: 6, y: 15, c: "#f6f1da" },
    { x: 6, y: 16, c: "#f6f1da" },
    { x: 6, y: 17, c: "#f6f1da" },
    { x: 7, y: 11, c: "#f6f1da" },
    { x: 7, y: 12, c: "#f6f1da" },
    { x: 7, y: 13, c: "#0a0d0c" },
    { x: 7, y: 14, c: "#0a0d0c" },
    { x: 7, y: 15, c: "#f6f1da" },
    { x: 7, y: 16, c: "#f6f1da" },
    { x: 7, y: 17, c: "#f6f1da" },
    { x: 8, y: 11, c: "#f6f1da" },
    { x: 8, y: 12, c: "#f6f1da" },
    { x: 8, y: 13, c: "#0a0d0c" },
    { x: 8, y: 14, c: "#0a0d0c" },
    { x: 8, y: 15, c: "#f6f1da" },
    { x: 8, y: 16, c: "#f6f1da" },
    { x: 8, y: 17, c: "#f6f1da" },
    { x: 9, y: 11, c: "#f6f1da" },
    { x: 9, y: 12, c: "#f6f1da" },
    { x: 9, y: 13, c: "#f6f1da" },
    { x: 9, y: 14, c: "#f6f1da" },
    { x: 9, y: 15, c: "#f6f1da" },
    { x: 9, y: 16, c: "#f6f1da" },
    { x: 9, y: 17, c: "#f6f1da" },
    { x: 10, y: 11, c: "#f6f1da" },
    { x: 10, y: 12, c: "#f6f1da" },
    { x: 10, y: 13, c: "#f6f1da" },
    { x: 10, y: 14, c: "#f6f1da" },
    { x: 10, y: 15, c: "#ee6fa4" },
    { x: 10, y: 16, c: "#f6f1da" },
    { x: 10, y: 17, c: "#f6f1da" },
    { x: 11, y: 11, c: "#f6f1da" },
    { x: 11, y: 12, c: "#f6f1da" },
    { x: 11, y: 13, c: "#f6f1da" },
    { x: 11, y: 14, c: "#f6f1da" },
    { x: 11, y: 15, c: "#f6f1da" },
    { x: 11, y: 16, c: "#f6f1da" },
    { x: 11, y: 17, c: "#f6f1da" },
    { x: 12, y: 11, c: "#f6f1da" },
    { x: 12, y: 12, c: "#f6f1da" },
    { x: 12, y: 13, c: "#0a0d0c" },
    { x: 12, y: 14, c: "#0a0d0c" },
    { x: 12, y: 15, c: "#f6f1da" },
    { x: 12, y: 16, c: "#f6f1da" },
    { x: 12, y: 17, c: "#f6f1da" },
    { x: 13, y: 12, c: "#f6f1da" },
    { x: 13, y: 13, c: "#0a0d0c" },
    { x: 13, y: 14, c: "#0a0d0c" },
    { x: 13, y: 15, c: "#f6f1da" },
    { x: 13, y: 16, c: "#f6f1da" },
    { x: 13, y: 17, c: "#f6f1da" },
    { x: 14, y: 12, c: "#f6f1da" },
    { x: 14, y: 13, c: "#f6f1da" },
    { x: 14, y: 14, c: "#f6f1da" },
    { x: 14, y: 15, c: "#f6f1da" },
    { x: 14, y: 16, c: "#f6f1da" },
    { x: 14, y: 17, c: "#f6f1da" },
    { x: 15, y: 12, c: "#f6f1da" },
    { x: 15, y: 13, c: "#f6f1da" },
    { x: 15, y: 14, c: "#f6f1da" },
    { x: 15, y: 15, c: "#f6f1da" },
    { x: 15, y: 16, c: "#f6f1da" },
    { x: 16, y: 13, c: "#f6f1da" },
    { x: 16, y: 14, c: "#f6f1da" },
    { x: 16, y: 15, c: "#f6f1da" },
    { x: 4, y: 13, c: "#f6f1da" },
    { x: 4, y: 14, c: "#f6f1da" },
    { x: 4, y: 15, c: "#f6f1da" },
    { x: 4, y: 16, c: "#f6f1da" },
    { x: 6, y: 19, c: "#f6f1da" },
    { x: 6, y: 20, c: "#f6f1da" },
    { x: 6, y: 21, c: "#f6f1da" },
    { x: 7, y: 18, c: "#f6f1da" },
    { x: 7, y: 19, c: "#f6f1da" },
    { x: 7, y: 20, c: "#f6f1da" },
    { x: 7, y: 21, c: "#f6f1da" },
    { x: 7, y: 22, c: "#f6f1da" },
    { x: 8, y: 18, c: "#f6f1da" },
    { x: 8, y: 19, c: "#f6f1da" },
    { x: 8, y: 20, c: "#f6f1da" },
    { x: 8, y: 21, c: "#f6f1da" },
    { x: 8, y: 22, c: "#f6f1da" },
    { x: 9, y: 18, c: "#f6f1da" },
    { x: 9, y: 19, c: "#f6f1da" },
    { x: 9, y: 20, c: "#f6f1da" },
    { x: 9, y: 21, c: "#f6f1da" },
    { x: 9, y: 22, c: "#f6f1da" },
    { x: 10, y: 18, c: "#f6f1da" },
    { x: 10, y: 19, c: "#f6f1da" },
    { x: 10, y: 20, c: "#f6f1da" },
    { x: 10, y: 21, c: "#f6f1da" },
    { x: 10, y: 22, c: "#f6f1da" },
    { x: 11, y: 18, c: "#f6f1da" },
    { x: 11, y: 19, c: "#f6f1da" },
    { x: 11, y: 20, c: "#f6f1da" },
    { x: 11, y: 21, c: "#f6f1da" },
    { x: 11, y: 22, c: "#f6f1da" },
    { x: 12, y: 18, c: "#f6f1da" },
    { x: 12, y: 19, c: "#f6f1da" },
    { x: 12, y: 20, c: "#f6f1da" },
    { x: 12, y: 21, c: "#f6f1da" },
    { x: 12, y: 22, c: "#f6f1da" },
    { x: 13, y: 18, c: "#f6f1da" },
    { x: 13, y: 19, c: "#f6f1da" },
    { x: 13, y: 20, c: "#f6f1da" },
    { x: 13, y: 21, c: "#f6f1da" },
    { x: 13, y: 22, c: "#f6f1da" },
    { x: 14, y: 18, c: "#f6f1da" },
    { x: 14, y: 19, c: "#f6f1da" },
    { x: 14, y: 20, c: "#f6f1da" },
    { x: 14, y: 21, c: "#f6f1da" },
    { x: 14, y: 22, c: "#f6f1da" },
    { x: 15, y: 19, c: "#f6f1da" },
    { x: 15, y: 20, c: "#f6f1da" },
    { x: 15, y: 21, c: "#f6f1da" },
    { x: 16, y: 20, c: "#f6f1da" },
    { x: 15, y: 17, c: "#f6f1da" },
    { x: 16, y: 16, c: "#f6f1da" },
    { x: 14, y: 6, c: "#f6f1da" },
    { x: 5, y: 6, c: "#f6f1da" },
    { x: 5, y: 7, c: "#f6f1da" },
    { x: 14, y: 10, c: "#f6f1da" },
    { x: 14, y: 9, c: "#f6f1da" },
    { x: 14, y: 8, c: "#f6f1da" },
    { x: 14, y: 7, c: "#f6f1da" },
    { x: 12, y: 6, c: "#f6f1da" },
    { x: 12, y: 7, c: "#f6f1da" },
    { x: 12, y: 8, c: "#f6f1da" },
    { x: 12, y: 9, c: "#f6f1da" },
    { x: 12, y: 10, c: "#f6f1da" },
    { x: 12, y: 5, c: "#f6f1da" },
    { x: 13, y: 5, c: "#f6f1da" },
    { x: 14, y: 5, c: "#f6f1da" },
  ],
  eyes: [
    { x1: 7, y1: 13, x2: 8, y2: 14 },
    { x1: 12, y1: 13, x2: 13, y2: 14 },
  ],
  lofi: true,
  parts: [
    { name: "earL", pivot: { x: 7, y: 10 }, cells: [{ x: 5, y: 6 }, { x: 5, y: 7 }, { x: 6, y: 6 }, { x: 6, y: 7 }, { x: 6, y: 8 }, { x: 6, y: 9 }, { x: 6, y: 10 }, { x: 7, y: 6 }, { x: 7, y: 7 }, { x: 7, y: 8 }, { x: 7, y: 9 }, { x: 7, y: 10 }, { x: 8, y: 6 }, { x: 8, y: 7 }, { x: 8, y: 8 }, { x: 8, y: 9 }, { x: 8, y: 10 }] },
    { name: "earR", pivot: { x: 13, y: 10 }, cells: [{ x: 12, y: 5 }, { x: 12, y: 6 }, { x: 12, y: 7 }, { x: 12, y: 8 }, { x: 12, y: 9 }, { x: 12, y: 10 }, { x: 13, y: 5 }, { x: 13, y: 6 }, { x: 13, y: 7 }, { x: 13, y: 8 }, { x: 13, y: 9 }, { x: 13, y: 10 }, { x: 14, y: 5 }, { x: 14, y: 6 }, { x: 14, y: 7 }, { x: 14, y: 8 }, { x: 14, y: 9 }, { x: 14, y: 10 }] },
  ],
  // Hand-drawn jump frame: celebrate flips idle stance <-> this pose while
  // confetti falls, the classic 2-frame Tamagotchi celebration.
  poses: [
    {
      name: "celebrate",
      cells: [
        { x: 6, y: 2, c: "#f6f1da" },
        { x: 6, y: 3, c: "#f6f1da" },
        { x: 6, y: 4, c: "#f6f1da" },
        { x: 6, y: 5, c: "#f6f1da" },
        { x: 6, y: 6, c: "#f6f1da" },
        { x: 7, y: 2, c: "#f6f1da" },
        { x: 7, y: 3, c: "#f4935e" },
        { x: 7, y: 4, c: "#f4935e" },
        { x: 7, y: 5, c: "#f4935e" },
        { x: 7, y: 6, c: "#f6f1da" },
        { x: 8, y: 2, c: "#f6f1da" },
        { x: 8, y: 3, c: "#f6f1da" },
        { x: 8, y: 4, c: "#f6f1da" },
        { x: 8, y: 5, c: "#f6f1da" },
        { x: 8, y: 6, c: "#f6f1da" },
        { x: 13, y: 2, c: "#f4935e" },
        { x: 13, y: 3, c: "#f4935e" },
        { x: 13, y: 4, c: "#f4935e" },
        { x: 13, y: 5, c: "#f4935e" },
        { x: 13, y: 6, c: "#f6f1da" },
        { x: 13, y: 7, c: "#f6f1da" },
        { x: 14, y: 7, c: "#f6f1da" },
        { x: 15, y: 7, c: "#f6f1da" },
        { x: 5, y: 7, c: "#f6f1da" },
        { x: 5, y: 8, c: "#f6f1da" },
        { x: 5, y: 9, c: "#f6f1da" },
        { x: 5, y: 10, c: "#f6f1da" },
        { x: 5, y: 11, c: "#f6f1da" },
        { x: 5, y: 12, c: "#f6f1da" },
        { x: 5, y: 13, c: "#f6f1da" },
        { x: 6, y: 7, c: "#f6f1da" },
        { x: 6, y: 8, c: "#f6f1da" },
        { x: 6, y: 9, c: "#f6f1da" },
        { x: 6, y: 10, c: "#f6f1da" },
        { x: 6, y: 11, c: "#f6f1da" },
        { x: 6, y: 12, c: "#f6f1da" },
        { x: 6, y: 13, c: "#f6f1da" },
        { x: 7, y: 7, c: "#f6f1da" },
        { x: 7, y: 8, c: "#f6f1da" },
        { x: 7, y: 9, c: "#0a0d0c" },
        { x: 7, y: 10, c: "#0a0d0c" },
        { x: 7, y: 11, c: "#f6f1da" },
        { x: 7, y: 12, c: "#f6f1da" },
        { x: 7, y: 13, c: "#f6f1da" },
        { x: 8, y: 7, c: "#f6f1da" },
        { x: 8, y: 8, c: "#f6f1da" },
        { x: 8, y: 9, c: "#0a0d0c" },
        { x: 8, y: 10, c: "#0a0d0c" },
        { x: 8, y: 11, c: "#f6f1da" },
        { x: 8, y: 12, c: "#f6f1da" },
        { x: 8, y: 13, c: "#f6f1da" },
        { x: 9, y: 7, c: "#f6f1da" },
        { x: 9, y: 8, c: "#f6f1da" },
        { x: 9, y: 9, c: "#f6f1da" },
        { x: 9, y: 10, c: "#f6f1da" },
        { x: 9, y: 11, c: "#f6f1da" },
        { x: 9, y: 12, c: "#f6f1da" },
        { x: 9, y: 13, c: "#f6f1da" },
        { x: 10, y: 7, c: "#f6f1da" },
        { x: 10, y: 8, c: "#f6f1da" },
        { x: 10, y: 9, c: "#f6f1da" },
        { x: 10, y: 10, c: "#f6f1da" },
        { x: 10, y: 11, c: "#ee6fa4" },
        { x: 10, y: 12, c: "#f6f1da" },
        { x: 10, y: 13, c: "#f6f1da" },
        { x: 11, y: 7, c: "#f6f1da" },
        { x: 11, y: 8, c: "#f6f1da" },
        { x: 11, y: 9, c: "#f6f1da" },
        { x: 11, y: 10, c: "#f6f1da" },
        { x: 11, y: 11, c: "#f6f1da" },
        { x: 11, y: 12, c: "#f6f1da" },
        { x: 11, y: 13, c: "#f6f1da" },
        { x: 12, y: 7, c: "#f6f1da" },
        { x: 12, y: 8, c: "#f6f1da" },
        { x: 12, y: 9, c: "#0a0d0c" },
        { x: 12, y: 10, c: "#0a0d0c" },
        { x: 12, y: 11, c: "#f6f1da" },
        { x: 12, y: 12, c: "#f6f1da" },
        { x: 12, y: 13, c: "#f6f1da" },
        { x: 13, y: 8, c: "#f6f1da" },
        { x: 13, y: 9, c: "#0a0d0c" },
        { x: 13, y: 10, c: "#0a0d0c" },
        { x: 13, y: 11, c: "#f6f1da" },
        { x: 13, y: 12, c: "#f6f1da" },
        { x: 13, y: 13, c: "#f6f1da" },
        { x: 14, y: 8, c: "#f6f1da" },
        { x: 14, y: 9, c: "#f6f1da" },
        { x: 14, y: 10, c: "#f6f1da" },
        { x: 14, y: 11, c: "#f6f1da" },
        { x: 14, y: 12, c: "#f6f1da" },
        { x: 14, y: 13, c: "#f6f1da" },
        { x: 15, y: 8, c: "#f6f1da" },
        { x: 15, y: 9, c: "#f6f1da" },
        { x: 15, y: 10, c: "#f6f1da" },
        { x: 15, y: 11, c: "#f6f1da" },
        { x: 15, y: 12, c: "#f6f1da" },
        { x: 16, y: 9, c: "#f6f1da" },
        { x: 16, y: 10, c: "#f6f1da" },
        { x: 16, y: 11, c: "#f6f1da" },
        { x: 4, y: 9, c: "#f6f1da" },
        { x: 4, y: 10, c: "#f6f1da" },
        { x: 4, y: 11, c: "#f6f1da" },
        { x: 4, y: 12, c: "#f6f1da" },
        { x: 6, y: 16, c: "#f6f1da" },
        { x: 6, y: 17, c: "#f6f1da" },
        { x: 7, y: 14, c: "#f6f1da" },
        { x: 7, y: 15, c: "#f6f1da" },
        { x: 7, y: 16, c: "#f6f1da" },
        { x: 7, y: 17, c: "#f6f1da" },
        { x: 7, y: 18, c: "#f6f1da" },
        { x: 8, y: 14, c: "#f6f1da" },
        { x: 8, y: 15, c: "#f6f1da" },
        { x: 8, y: 16, c: "#f6f1da" },
        { x: 8, y: 17, c: "#f6f1da" },
        { x: 8, y: 18, c: "#f6f1da" },
        { x: 9, y: 14, c: "#f6f1da" },
        { x: 9, y: 15, c: "#f6f1da" },
        { x: 9, y: 16, c: "#f6f1da" },
        { x: 9, y: 17, c: "#f6f1da" },
        { x: 9, y: 18, c: "#f6f1da" },
        { x: 10, y: 14, c: "#f6f1da" },
        { x: 10, y: 15, c: "#f6f1da" },
        { x: 10, y: 16, c: "#f6f1da" },
        { x: 10, y: 17, c: "#f6f1da" },
        { x: 10, y: 18, c: "#f6f1da" },
        { x: 11, y: 14, c: "#f6f1da" },
        { x: 11, y: 15, c: "#f6f1da" },
        { x: 11, y: 16, c: "#f6f1da" },
        { x: 11, y: 17, c: "#f6f1da" },
        { x: 11, y: 18, c: "#f6f1da" },
        { x: 12, y: 14, c: "#f6f1da" },
        { x: 12, y: 15, c: "#f6f1da" },
        { x: 12, y: 16, c: "#f6f1da" },
        { x: 12, y: 17, c: "#f6f1da" },
        { x: 12, y: 18, c: "#f6f1da" },
        { x: 13, y: 14, c: "#f6f1da" },
        { x: 13, y: 15, c: "#f6f1da" },
        { x: 13, y: 16, c: "#f6f1da" },
        { x: 13, y: 17, c: "#f6f1da" },
        { x: 13, y: 18, c: "#f6f1da" },
        { x: 14, y: 14, c: "#f6f1da" },
        { x: 14, y: 15, c: "#f6f1da" },
        { x: 14, y: 16, c: "#f6f1da" },
        { x: 14, y: 17, c: "#f6f1da" },
        { x: 14, y: 18, c: "#f6f1da" },
        { x: 15, y: 15, c: "#f6f1da" },
        { x: 15, y: 16, c: "#f6f1da" },
        { x: 15, y: 17, c: "#f6f1da" },
        { x: 16, y: 16, c: "#f6f1da" },
        { x: 15, y: 13, c: "#f6f1da" },
        { x: 16, y: 12, c: "#f6f1da" },
        { x: 14, y: 2, c: "#f6f1da" },
        { x: 5, y: 2, c: "#f6f1da" },
        { x: 5, y: 3, c: "#f6f1da" },
        { x: 14, y: 6, c: "#f6f1da" },
        { x: 14, y: 5, c: "#f6f1da" },
        { x: 14, y: 4, c: "#f6f1da" },
        { x: 14, y: 3, c: "#f6f1da" },
        { x: 12, y: 2, c: "#f6f1da" },
        { x: 12, y: 3, c: "#f6f1da" },
        { x: 12, y: 4, c: "#f6f1da" },
        { x: 12, y: 5, c: "#f6f1da" },
        { x: 12, y: 6, c: "#f6f1da" },
        { x: 12, y: 1, c: "#f6f1da" },
        { x: 13, y: 1, c: "#f6f1da" },
        { x: 14, y: 1, c: "#f6f1da" },
        { x: 16, y: 14, c: "#f6f1da" },
        { x: 16, y: 13, c: "#f6f1da" },
        { x: 17, y: 13, c: "#f6f1da" },
        { x: 17, y: 12, c: "#f6f1da" },
        { x: 18, y: 12, c: "#f6f1da" },
        { x: 19, y: 12, c: "#f6f1da" },
        { x: 19, y: 11, c: "#f6f1da" },
        { x: 18, y: 11, c: "#f6f1da" },
        { x: 15, y: 14, c: "#f6f1da" },
        { x: 18, y: 13, c: "#f6f1da" },
        { x: 17, y: 14, c: "#f6f1da" },
        { x: 6, y: 14, c: "#f6f1da" },
        { x: 2, y: 12, c: "#f6f1da" },
        { x: 1, y: 12, c: "#f6f1da" },
        { x: 2, y: 13, c: "#f6f1da" },
        { x: 3, y: 14, c: "#f6f1da" },
        { x: 5, y: 14, c: "#f6f1da" },
        { x: 4, y: 14, c: "#f6f1da" },
        { x: 3, y: 13, c: "#f6f1da" },
        { x: 4, y: 13, c: "#f6f1da" },
        { x: 3, y: 12, c: "#f6f1da" },
        { x: 6, y: 15, c: "#f6f1da" },
        { x: 1, y: 11, c: "#f6f1da" },
        { x: 2, y: 11, c: "#f6f1da" },
        { x: 8, y: 19, c: "#f6f1da" },
        { x: 8, y: 20, c: "#f6f1da" },
        { x: 9, y: 20, c: "#f6f1da" },
        { x: 9, y: 19, c: "#f6f1da" },
        { x: 13, y: 19, c: "#f6f1da" },
        { x: 12, y: 20, c: "#f6f1da" },
        { x: 13, y: 20, c: "#f6f1da" },
        { x: 12, y: 19, c: "#f6f1da" },
      ],
    },
  ],
});

const duck = (): PixelModel => ({
  id: "duck",
  name: "Puddle the Duck",
  description: "A snowy duck with a two-tone tangerine bill and webbed feet.",
  cells: [
    { x: 3, y: 1, c: "#f8f6ea" },
    { x: 3, y: 2, c: "#f8f6ea" },
    { x: 3, y: 3, c: "#f8f6ea" },
    { x: 3, y: 4, c: "#f8f6ea" },
    { x: 3, y: 5, c: "#f8f6ea" },
    { x: 3, y: 6, c: "#f48f6d" },
    { x: 3, y: 7, c: "#f48f6d" },
    { x: 3, y: 8, c: "#ee6e44" },
    { x: 3, y: 9, c: "#ee6e44" },
    { x: 3, y: 10, c: "#f8f6ea" },
    { x: 3, y: 11, c: "#f8f6ea" },
    { x: 3, y: 12, c: "#f8f6ea" },
    { x: 3, y: 13, c: "#f8f6ea" },
    { x: 3, y: 14, c: "#f8f6ea" },
    { x: 4, y: 1, c: "#f8f6ea" },
    { x: 4, y: 2, c: "#f8f6ea" },
    { x: 4, y: 3, c: "#0a0d0c" },
    { x: 4, y: 4, c: "#0a0d0c" },
    { x: 4, y: 5, c: "#0a0d0c" },
    { x: 4, y: 6, c: "#f48f6d" },
    { x: 4, y: 7, c: "#f48f6d" },
    { x: 4, y: 8, c: "#ee6e44" },
    { x: 4, y: 9, c: "#ee6e44" },
    { x: 4, y: 10, c: "#f8f6ea" },
    { x: 4, y: 11, c: "#f8f6ea" },
    { x: 4, y: 12, c: "#f8f6ea" },
    { x: 4, y: 13, c: "#f8f6ea" },
    { x: 4, y: 14, c: "#f8f6ea" },
    { x: 5, y: 1, c: "#f8f6ea" },
    { x: 5, y: 2, c: "#f8f6ea" },
    { x: 5, y: 3, c: "#0a0d0c" },
    { x: 5, y: 4, c: "#0a0d0c" },
    { x: 5, y: 5, c: "#0a0d0c" },
    { x: 5, y: 6, c: "#f48f6d" },
    { x: 5, y: 7, c: "#f48f6d" },
    { x: 5, y: 8, c: "#ee6e44" },
    { x: 5, y: 9, c: "#ee6e44" },
    { x: 5, y: 10, c: "#f8f6ea" },
    { x: 5, y: 11, c: "#f8f6ea" },
    { x: 5, y: 12, c: "#f8f6ea" },
    { x: 5, y: 13, c: "#f8f6ea" },
    { x: 5, y: 14, c: "#f8f6ea" },
    { x: 6, y: 1, c: "#f8f6ea" },
    { x: 6, y: 2, c: "#f8f6ea" },
    { x: 6, y: 3, c: "#f8f6ea" },
    { x: 6, y: 4, c: "#f8f6ea" },
    { x: 6, y: 5, c: "#f8f6ea" },
    { x: 6, y: 6, c: "#f48f6d" },
    { x: 6, y: 7, c: "#f48f6d" },
    { x: 6, y: 8, c: "#ee6e44" },
    { x: 6, y: 9, c: "#ee6e44" },
    { x: 6, y: 10, c: "#f8f6ea" },
    { x: 6, y: 11, c: "#f8f6ea" },
    { x: 6, y: 12, c: "#f8f6ea" },
    { x: 6, y: 13, c: "#f8f6ea" },
    { x: 6, y: 14, c: "#f8f6ea" },
    { x: 7, y: 1, c: "#f8f6ea" },
    { x: 7, y: 2, c: "#f8f6ea" },
    { x: 7, y: 3, c: "#f8f6ea" },
    { x: 7, y: 4, c: "#f8f6ea" },
    { x: 7, y: 5, c: "#f8f6ea" },
    { x: 7, y: 6, c: "#f8f6ea" },
    { x: 7, y: 7, c: "#f8f6ea" },
    { x: 7, y: 8, c: "#ee6e44" },
    { x: 7, y: 9, c: "#ee6e44" },
    { x: 7, y: 10, c: "#f8f6ea" },
    { x: 7, y: 11, c: "#f8f6ea" },
    { x: 7, y: 12, c: "#f8f6ea" },
    { x: 7, y: 13, c: "#f8f6ea" },
    { x: 7, y: 14, c: "#f8f6ea" },
    { x: 8, y: 1, c: "#f8f6ea" },
    { x: 8, y: 2, c: "#f8f6ea" },
    { x: 8, y: 3, c: "#f8f6ea" },
    { x: 8, y: 4, c: "#f8f6ea" },
    { x: 8, y: 5, c: "#f8f6ea" },
    { x: 8, y: 6, c: "#f8f6ea" },
    { x: 8, y: 7, c: "#f8f6ea" },
    { x: 8, y: 8, c: "#f8f6ea" },
    { x: 8, y: 9, c: "#f8f6ea" },
    { x: 8, y: 10, c: "#f8f6ea" },
    { x: 8, y: 11, c: "#f8f6ea" },
    { x: 8, y: 12, c: "#f8f6ea" },
    { x: 8, y: 13, c: "#f8f6ea" },
    { x: 8, y: 14, c: "#f8f6ea" },
    { x: 9, y: 1, c: "#f8f6ea" },
    { x: 9, y: 2, c: "#f8f6ea" },
    { x: 9, y: 3, c: "#0a0d0c" },
    { x: 9, y: 4, c: "#0a0d0c" },
    { x: 9, y: 5, c: "#0a0d0c" },
    { x: 9, y: 6, c: "#f8f6ea" },
    { x: 9, y: 7, c: "#f8f6ea" },
    { x: 9, y: 8, c: "#f8f6ea" },
    { x: 9, y: 9, c: "#f8f6ea" },
    { x: 9, y: 10, c: "#f8f6ea" },
    { x: 9, y: 11, c: "#f8f6ea" },
    { x: 9, y: 12, c: "#f8f6ea" },
    { x: 9, y: 13, c: "#f8f6ea" },
    { x: 9, y: 14, c: "#f8f6ea" },
    { x: 10, y: 1, c: "#f8f6ea" },
    { x: 10, y: 2, c: "#f8f6ea" },
    { x: 10, y: 3, c: "#0a0d0c" },
    { x: 10, y: 4, c: "#0a0d0c" },
    { x: 10, y: 5, c: "#0a0d0c" },
    { x: 10, y: 6, c: "#f8f6ea" },
    { x: 10, y: 7, c: "#f8f6ea" },
    { x: 10, y: 8, c: "#f8f6ea" },
    { x: 10, y: 9, c: "#f8f6ea" },
    { x: 10, y: 10, c: "#f8f6ea" },
    { x: 10, y: 11, c: "#f8f6ea" },
    { x: 10, y: 12, c: "#f8f6ea" },
    { x: 10, y: 13, c: "#f8f6ea" },
    { x: 10, y: 14, c: "#f8f6ea" },
    { x: 11, y: 1, c: "#f8f6ea" },
    { x: 11, y: 2, c: "#f8f6ea" },
    { x: 11, y: 3, c: "#f8f6ea" },
    { x: 11, y: 4, c: "#f8f6ea" },
    { x: 11, y: 5, c: "#f8f6ea" },
    { x: 11, y: 6, c: "#f8f6ea" },
    { x: 11, y: 7, c: "#f8f6ea" },
    { x: 11, y: 8, c: "#f8f6ea" },
    { x: 11, y: 9, c: "#f8f6ea" },
    { x: 11, y: 10, c: "#f8f6ea" },
    { x: 11, y: 11, c: "#f8f6ea" },
    { x: 11, y: 12, c: "#f8f6ea" },
    { x: 11, y: 13, c: "#f8f6ea" },
    { x: 11, y: 14, c: "#f8f6ea" },
    { x: 12, y: 3, c: "#f8f6ea" },
    { x: 12, y: 4, c: "#f8f6ea" },
    { x: 12, y: 5, c: "#f8f6ea" },
    { x: 12, y: 6, c: "#f8f6ea" },
    { x: 12, y: 7, c: "#f8f6ea" },
    { x: 12, y: 8, c: "#f8f6ea" },
    { x: 12, y: 9, c: "#f8f6ea" },
    { x: 12, y: 10, c: "#f8f6ea" },
    { x: 12, y: 11, c: "#f8f6ea" },
    { x: 12, y: 12, c: "#f8f6ea" },
    { x: 12, y: 13, c: "#f8f6ea" },
    { x: 12, y: 14, c: "#f8f6ea" },
    { x: 2, y: 3, c: "#f8f6ea" },
    { x: 2, y: 4, c: "#f8f6ea" },
    { x: 2, y: 5, c: "#f8f6ea" },
    { x: 2, y: 6, c: "#f48f6d" },
    { x: 2, y: 7, c: "#f48f6d" },
    { x: 2, y: 8, c: "#ee6e44" },
    { x: 2, y: 9, c: "#ee6e44" },
    { x: 13, y: 11, c: "#f8f6ea" },
    { x: 13, y: 12, c: "#f8f6ea" },
    { x: 1, y: 6, c: "#f48f6d" },
    { x: 1, y: 7, c: "#f48f6d" },
    { x: 1, y: 8, c: "#ee6e44" },
    { x: 1, y: 9, c: "#ee6e44" },
    { x: 14, y: 11, c: "#f8f6ea" },
    { x: 15, y: 11, c: "#f8f6ea" },
    { x: 16, y: 11, c: "#f8f6ea" },
    { x: 16, y: 12, c: "#f8f6ea" },
    { x: 16, y: 13, c: "#f8f6ea" },
    { x: 16, y: 14, c: "#f8f6ea" },
    { x: 16, y: 15, c: "#f8f6ea" },
    { x: 16, y: 16, c: "#f8f6ea" },
    { x: 15, y: 17, c: "#f8f6ea" },
    { x: 15, y: 18, c: "#f8f6ea" },
    { x: 14, y: 18, c: "#f8f6ea" },
    { x: 13, y: 18, c: "#f8f6ea" },
    { x: 12, y: 18, c: "#f8f6ea" },
    { x: 11, y: 18, c: "#f8f6ea" },
    { x: 10, y: 18, c: "#f8f6ea" },
    { x: 9, y: 18, c: "#f8f6ea" },
    { x: 8, y: 18, c: "#f8f6ea" },
    { x: 7, y: 18, c: "#f8f6ea" },
    { x: 6, y: 18, c: "#f8f6ea" },
    { x: 5, y: 18, c: "#f8f6ea" },
    { x: 4, y: 18, c: "#f8f6ea" },
    { x: 3, y: 17, c: "#f8f6ea" },
    { x: 3, y: 16, c: "#f8f6ea" },
    { x: 3, y: 15, c: "#f8f6ea" },
    { x: 4, y: 15, c: "#f8f6ea" },
    { x: 5, y: 15, c: "#f8f6ea" },
    { x: 6, y: 15, c: "#f8f6ea" },
    { x: 7, y: 15, c: "#f8f6ea" },
    { x: 8, y: 15, c: "#f8f6ea" },
    { x: 9, y: 15, c: "#f8f6ea" },
    { x: 10, y: 15, c: "#f8f6ea" },
    { x: 11, y: 15, c: "#f8f6ea" },
    { x: 12, y: 15, c: "#f8f6ea" },
    { x: 13, y: 15, c: "#f8f6ea" },
    { x: 14, y: 15, c: "#f8f6ea" },
    { x: 14, y: 14, c: "#f8f6ea" },
    { x: 15, y: 13, c: "#f8f6ea" },
    { x: 14, y: 13, c: "#f8f6ea" },
    { x: 13, y: 14, c: "#f8f6ea" },
    { x: 13, y: 13, c: "#f8f6ea" },
    { x: 14, y: 12, c: "#f8f6ea" },
    { x: 15, y: 12, c: "#f8f6ea" },
    { x: 15, y: 14, c: "#f8f6ea" },
    { x: 15, y: 15, c: "#f8f6ea" },
    { x: 15, y: 16, c: "#f8f6ea" },
    { x: 14, y: 16, c: "#f8f6ea" },
    { x: 14, y: 17, c: "#f8f6ea" },
    { x: 13, y: 17, c: "#f8f6ea" },
    { x: 12, y: 17, c: "#f8f6ea" },
    { x: 11, y: 17, c: "#f8f6ea" },
    { x: 10, y: 17, c: "#f8f6ea" },
    { x: 9, y: 17, c: "#f8f6ea" },
    { x: 8, y: 17, c: "#f8f6ea" },
    { x: 7, y: 17, c: "#f8f6ea" },
    { x: 6, y: 17, c: "#f8f6ea" },
    { x: 5, y: 17, c: "#f8f6ea" },
    { x: 4, y: 17, c: "#f8f6ea" },
    { x: 4, y: 16, c: "#f8f6ea" },
    { x: 5, y: 16, c: "#f8f6ea" },
    { x: 6, y: 16, c: "#f8f6ea" },
    { x: 7, y: 16, c: "#f8f6ea" },
    { x: 8, y: 16, c: "#f8f6ea" },
    { x: 9, y: 16, c: "#f8f6ea" },
    { x: 10, y: 16, c: "#f8f6ea" },
    { x: 11, y: 16, c: "#f8f6ea" },
    { x: 12, y: 16, c: "#f8f6ea" },
    { x: 13, y: 16, c: "#f8f6ea" },
    { x: 17, y: 10, c: "#f8f6ea" },
    { x: 17, y: 11, c: "#f8f6ea" },
    { x: 17, y: 12, c: "#f8f6ea" },
    { x: 17, y: 13, c: "#f8f6ea" },
    { x: 17, y: 14, c: "#f8f6ea" },
    { x: 5, y: 19, c: "#ee6e44" },
    { x: 6, y: 19, c: "#ee6e44" },
    { x: 7, y: 19, c: "#ee6e44" },
    { x: 10, y: 19, c: "#ee6e44" },
    { x: 11, y: 19, c: "#ee6e44" },
    { x: 12, y: 19, c: "#ee6e44" },
    { x: 0, y: 7, c: "#f48f6d" },
    { x: 0, y: 6, c: "#f48f6d" },
    { x: 0, y: 8, c: "#ee6e44" },
    { x: 0, y: 9, c: "#ee6e44" },
  ],
  eyes: [
    { x1: 4, y1: 3, x2: 5, y2: 5 },
    { x1: 9, y1: 3, x2: 10, y2: 5 },
  ],
  // Lo-fi body reactions + mood props. No rigged parts — the duck has no
  // ears to flop.
  lofi: true,
});

// Hand-tuned in the critter editor (/preview/critter-editor).
const cat = (): PixelModel => ({
  id: "cat",
  name: "Smokey the Cat",
  description: "A gray cat with whiskery cheeks, a pink nose and a curled tail.",
  cells: [
    { x: 3, y: 4, c: "#bfbfba" },
    { x: 4, y: 4, c: "#bfbfba" },
    { x: 5, y: 4, c: "#bfbfba" },
    { x: 6, y: 4, c: "#bfbfba" },
    { x: 10, y: 4, c: "#bfbfba" },
    { x: 11, y: 4, c: "#bfbfba" },
    { x: 12, y: 4, c: "#bfbfba" },
    { x: 13, y: 4, c: "#bfbfba" },
    { x: 2, y: 4, c: "#bfbfba" },
    { x: 2, y: 5, c: "#bfbfba" },
    { x: 2, y: 6, c: "#92928b" },
    { x: 2, y: 7, c: "#bfbfba" },
    { x: 2, y: 8, c: "#92928b" },
    { x: 2, y: 9, c: "#bfbfba" },
    { x: 2, y: 10, c: "#bfbfba" },
    { x: 3, y: 5, c: "#bfbfba" },
    { x: 3, y: 6, c: "#bfbfba" },
    { x: 3, y: 7, c: "#bfbfba" },
    { x: 3, y: 8, c: "#bfbfba" },
    { x: 3, y: 9, c: "#bfbfba" },
    { x: 3, y: 10, c: "#bfbfba" },
    { x: 3, y: 11, c: "#bfbfba" },
    { x: 4, y: 5, c: "#bfbfba" },
    { x: 4, y: 6, c: "#0a0d0c" },
    { x: 4, y: 7, c: "#0a0d0c" },
    { x: 4, y: 8, c: "#bfbfba" },
    { x: 4, y: 9, c: "#bfbfba" },
    { x: 4, y: 10, c: "#bfbfba" },
    { x: 4, y: 11, c: "#bfbfba" },
    { x: 5, y: 5, c: "#bfbfba" },
    { x: 5, y: 6, c: "#0a0d0c" },
    { x: 5, y: 7, c: "#0a0d0c" },
    { x: 5, y: 8, c: "#bfbfba" },
    { x: 5, y: 9, c: "#bfbfba" },
    { x: 5, y: 10, c: "#bfbfba" },
    { x: 5, y: 11, c: "#bfbfba" },
    { x: 6, y: 5, c: "#bfbfba" },
    { x: 6, y: 6, c: "#bfbfba" },
    { x: 6, y: 7, c: "#bfbfba" },
    { x: 6, y: 8, c: "#bfbfba" },
    { x: 6, y: 9, c: "#bfbfba" },
    { x: 6, y: 10, c: "#bfbfba" },
    { x: 6, y: 11, c: "#bfbfba" },
    { x: 7, y: 4, c: "#92928b" },
    { x: 7, y: 5, c: "#bfbfba" },
    { x: 7, y: 6, c: "#bfbfba" },
    { x: 7, y: 7, c: "#bfbfba" },
    { x: 7, y: 8, c: "#bfbfba" },
    { x: 7, y: 9, c: "#ebbcbc" },
    { x: 7, y: 10, c: "#bfbfba" },
    { x: 7, y: 11, c: "#bfbfba" },
    { x: 8, y: 4, c: "#bfbfba" },
    { x: 8, y: 5, c: "#bfbfba" },
    { x: 8, y: 6, c: "#bfbfba" },
    { x: 8, y: 7, c: "#bfbfba" },
    { x: 8, y: 8, c: "#bfbfba" },
    { x: 8, y: 9, c: "#bfbfba" },
    { x: 8, y: 10, c: "#bfbfba" },
    { x: 8, y: 11, c: "#bfbfba" },
    { x: 9, y: 4, c: "#bfbfba" },
    { x: 9, y: 5, c: "#bfbfba" },
    { x: 9, y: 6, c: "#0a0d0c" },
    { x: 9, y: 8, c: "#bfbfba" },
    { x: 9, y: 9, c: "#bfbfba" },
    { x: 9, y: 10, c: "#bfbfba" },
    { x: 9, y: 11, c: "#bfbfba" },
    { x: 10, y: 5, c: "#bfbfba" },
    { x: 10, y: 6, c: "#0a0d0c" },
    { x: 10, y: 7, c: "#0a0d0c" },
    { x: 10, y: 9, c: "#bfbfba" },
    { x: 10, y: 10, c: "#bfbfba" },
    { x: 10, y: 11, c: "#bfbfba" },
    { x: 11, y: 5, c: "#bfbfba" },
    { x: 11, y: 6, c: "#bfbfba" },
    { x: 11, y: 7, c: "#bfbfba" },
    { x: 11, y: 8, c: "#bfbfba" },
    { x: 11, y: 11, c: "#bfbfba" },
    { x: 12, y: 5, c: "#bfbfba" },
    { x: 12, y: 6, c: "#bfbfba" },
    { x: 12, y: 7, c: "#bfbfba" },
    { x: 12, y: 8, c: "#bfbfba" },
    { x: 12, y: 9, c: "#bfbfba" },
    { x: 13, y: 5, c: "#bfbfba" },
    { x: 13, y: 6, c: "#92928b" },
    { x: 13, y: 7, c: "#bfbfba" },
    { x: 13, y: 8, c: "#92928b" },
    { x: 13, y: 9, c: "#bfbfba" },
    { x: 13, y: 10, c: "#bfbfba" },
    { x: 4, y: 12, c: "#bfbfba" },
    { x: 4, y: 13, c: "#bfbfba" },
    { x: 4, y: 14, c: "#bfbfba" },
    { x: 4, y: 15, c: "#bfbfba" },
    { x: 4, y: 16, c: "#bfbfba" },
    { x: 4, y: 17, c: "#bfbfba" },
    { x: 4, y: 18, c: "#bfbfba" },
    { x: 4, y: 19, c: "#bfbfba" },
    { x: 5, y: 12, c: "#bfbfba" },
    { x: 5, y: 13, c: "#bfbfba" },
    { x: 5, y: 14, c: "#bfbfba" },
    { x: 5, y: 15, c: "#bfbfba" },
    { x: 5, y: 16, c: "#bfbfba" },
    { x: 5, y: 17, c: "#bfbfba" },
    { x: 5, y: 18, c: "#bfbfba" },
    { x: 5, y: 19, c: "#bfbfba" },
    { x: 6, y: 12, c: "#bfbfba" },
    { x: 6, y: 13, c: "#bfbfba" },
    { x: 6, y: 14, c: "#bfbfba" },
    { x: 6, y: 15, c: "#bfbfba" },
    { x: 6, y: 16, c: "#bfbfba" },
    { x: 6, y: 17, c: "#bfbfba" },
    { x: 6, y: 18, c: "#bfbfba" },
    { x: 6, y: 19, c: "#bfbfba" },
    { x: 7, y: 12, c: "#bfbfba" },
    { x: 7, y: 13, c: "#bfbfba" },
    { x: 7, y: 14, c: "#bfbfba" },
    { x: 7, y: 15, c: "#bfbfba" },
    { x: 7, y: 16, c: "#bfbfba" },
    { x: 7, y: 17, c: "#bfbfba" },
    { x: 7, y: 18, c: "#bfbfba" },
    { x: 7, y: 19, c: "#bfbfba" },
    { x: 8, y: 12, c: "#bfbfba" },
    { x: 8, y: 13, c: "#bfbfba" },
    { x: 8, y: 14, c: "#bfbfba" },
    { x: 8, y: 15, c: "#bfbfba" },
    { x: 8, y: 16, c: "#bfbfba" },
    { x: 8, y: 17, c: "#bfbfba" },
    { x: 8, y: 18, c: "#bfbfba" },
    { x: 8, y: 19, c: "#bfbfba" },
    { x: 9, y: 12, c: "#bfbfba" },
    { x: 9, y: 13, c: "#bfbfba" },
    { x: 9, y: 14, c: "#bfbfba" },
    { x: 9, y: 15, c: "#bfbfba" },
    { x: 9, y: 16, c: "#bfbfba" },
    { x: 9, y: 17, c: "#bfbfba" },
    { x: 9, y: 18, c: "#bfbfba" },
    { x: 9, y: 19, c: "#bfbfba" },
    { x: 10, y: 12, c: "#bfbfba" },
    { x: 10, y: 13, c: "#bfbfba" },
    { x: 10, y: 14, c: "#bfbfba" },
    { x: 10, y: 15, c: "#bfbfba" },
    { x: 10, y: 16, c: "#bfbfba" },
    { x: 10, y: 17, c: "#bfbfba" },
    { x: 10, y: 18, c: "#bfbfba" },
    { x: 10, y: 19, c: "#bfbfba" },
    { x: 11, y: 12, c: "#92928b" },
    { x: 11, y: 13, c: "#bfbfba" },
    { x: 11, y: 14, c: "#92928b" },
    { x: 11, y: 15, c: "#bfbfba" },
    { x: 11, y: 16, c: "#bfbfba" },
    { x: 11, y: 17, c: "#bfbfba" },
    { x: 11, y: 18, c: "#bfbfba" },
    { x: 11, y: 19, c: "#bfbfba" },
    { x: 12, y: 17, c: "#bfbfba" },
    { x: 12, y: 18, c: "#bfbfba" },
    { x: 12, y: 19, c: "#bfbfba" },
    { x: 15, y: 12, c: "#bfbfba" },
    { x: 15, y: 13, c: "#bfbfba" },
    { x: 15, y: 14, c: "#bfbfba" },
    { x: 15, y: 15, c: "#bfbfba" },
    { x: 15, y: 16, c: "#bfbfba" },
    { x: 16, y: 12, c: "#92928b" },
    { x: 16, y: 13, c: "#bfbfba" },
    { x: 16, y: 14, c: "#92928b" },
    { x: 16, y: 15, c: "#bfbfba" },
    { x: 16, y: 16, c: "#bfbfba" },
    { x: 16, y: 11, c: "#bfbfba" },
    { x: 13, y: 17, c: "#bfbfba" },
    { x: 13, y: 18, c: "#bfbfba" },
    { x: 13, y: 19, c: "#bfbfba" },
    { x: 14, y: 17, c: "#bfbfba" },
    { x: 14, y: 18, c: "#bfbfba" },
    { x: 14, y: 19, c: "#bfbfba" },
    { x: 15, y: 17, c: "#bfbfba" },
    { x: 15, y: 18, c: "#bfbfba" },
    { x: 15, y: 19, c: "#bfbfba" },
    { x: 16, y: 17, c: "#bfbfba" },
    { x: 1, y: 9, c: "#bfbfba" },
    { x: 1, y: 8, c: "#92928b" },
    { x: 1, y: 7, c: "#bfbfba" },
    { x: 1, y: 6, c: "#92928b" },
    { x: 1, y: 5, c: "#bfbfba" },
    { x: 14, y: 9, c: "#bfbfba" },
    { x: 14, y: 8, c: "#92928b" },
    { x: 14, y: 7, c: "#bfbfba" },
    { x: 14, y: 6, c: "#92928b" },
    { x: 14, y: 5, c: "#bfbfba" },
    { x: 2, y: 3, c: "#bfbfba" },
    { x: 2, y: 2, c: "#ebbcbc" },
    { x: 3, y: 3, c: "#bfbfba" },
    { x: 2, y: 1, c: "#ebbcbc" },
    { x: 3, y: 2, c: "#bfbfba" },
    { x: 4, y: 3, c: "#92928b" },
    { x: 13, y: 3, c: "#bfbfba" },
    { x: 13, y: 2, c: "#ebbcbc" },
    { x: 13, y: 1, c: "#ebbcbc" },
    { x: 12, y: 2, c: "#bfbfba" },
    { x: 12, y: 3, c: "#bfbfba" },
    { x: 11, y: 3, c: "#bfbfba" },
    { x: 6, y: 3, c: "#bfbfba" },
    { x: 7, y: 3, c: "#92928b" },
    { x: 8, y: 3, c: "#bfbfba" },
    { x: 9, y: 3, c: "#bfbfba" },
    { x: 7, y: 2, c: "#92928b" },
    { x: 8, y: 2, c: "#bfbfba" },
    { x: 5, y: 3, c: "#bfbfba" },
    { x: 10, y: 3, c: "#92928b" },
    { x: 6, y: 2, c: "#bfbfba" },
    { x: 9, y: 2, c: "#bfbfba" },
    { x: 11, y: 2, c: "#bfbfba" },
    { x: 12, y: 1, c: "#bfbfba" },
    { x: 13, y: 0, c: "#ebbcbc" },
    { x: 4, y: 2, c: "#92928b" },
    { x: 3, y: 1, c: "#bfbfba" },
    { x: 2, y: 0, c: "#ebbcbc" },
    { x: 12, y: 14, c: "#92928b" },
    { x: 12, y: 15, c: "#bfbfba" },
    { x: 12, y: 16, c: "#bfbfba" },
    { x: 13, y: 16, c: "#bfbfba" },
    { x: 14, y: 16, c: "#bfbfba" },
    { x: 3, y: 14, c: "#bfbfba" },
    { x: 3, y: 16, c: "#bfbfba" },
    { x: 3, y: 15, c: "#bfbfba" },
    { x: 3, y: 17, c: "#bfbfba" },
    { x: 3, y: 18, c: "#bfbfba" },
    { x: 11, y: 10, c: "#bfbfba" },
    { x: 12, y: 10, c: "#bfbfba" },
    { x: 11, y: 9, c: "#bfbfba" },
    { x: 10, y: 8, c: "#bfbfba" },
    { x: 9, y: 7, c: "#0a0d0c" },
    { x: 5, y: 2, c: "#bfbfba" },
    { x: 10, y: 2, c: "#92928b" },
    { x: 15, y: 6, c: "#92928b" },
    { x: 15, y: 8, c: "#92928b" },
    { x: 0, y: 8, c: "#92928b" },
    { x: 0, y: 6, c: "#92928b" },
    { x: 13, y: 15, c: "#bfbfba" },
  ],
  eyes: [
    { x1: 4, y1: 6, x2: 5, y2: 7 },
    { x1: 9, y1: 6, x2: 10, y2: 7 },
  ],
  // Same lo-fi reaction set as the bunny. The ear nubs skew from their base
  // row, which stays put, so the head band underneath never gapes.
  lofi: true,
  parts: [
    {
      name: "earL",
      pivot: { x: 2, y: 3 },
      cells: [
        { x: 2, y: 0 },
        { x: 2, y: 1 }, { x: 3, y: 1 },
        { x: 2, y: 2 },
      ],
    },
    {
      name: "earR",
      pivot: { x: 13, y: 3 },
      cells: [
        { x: 13, y: 0 },
        { x: 12, y: 1 }, { x: 13, y: 1 },
        { x: 13, y: 2 },
      ],
    },
  ],
});

// Hand-tuned in the critter editor (/preview/critter-editor).
const fox = (): PixelModel => ({
  id: "fox",
  name: "Pumpkin the Fox",
  description: "An orange fox with a cream muzzle, chest and tail tip.",
  cells: [
    { x: 2, y: 2, c: "#ffe5ad" },
    { x: 2, y: 3, c: "#ffe5ad" },
    { x: 3, y: 2, c: "#f0854f" },
    { x: 3, y: 3, c: "#f0854f" },
    { x: 4, y: 3, c: "#f0854f" },
    { x: 10, y: 3, c: "#f0854f" },
    { x: 11, y: 2, c: "#f0854f" },
    { x: 11, y: 3, c: "#f0854f" },
    { x: 12, y: 2, c: "#ffe5ad" },
    { x: 12, y: 3, c: "#ffe5ad" },
    { x: 1, y: 6, c: "#f0854f" },
    { x: 1, y: 7, c: "#f0854f" },
    { x: 1, y: 8, c: "#f0854f" },
    { x: 1, y: 9, c: "#f0854f" },
    { x: 2, y: 4, c: "#f0854f" },
    { x: 2, y: 5, c: "#f0854f" },
    { x: 2, y: 6, c: "#f0854f" },
    { x: 2, y: 7, c: "#f0854f" },
    { x: 2, y: 8, c: "#ffe5ad" },
    { x: 2, y: 9, c: "#ffe5ad" },
    { x: 2, y: 10, c: "#f0854f" },
    { x: 3, y: 4, c: "#f0854f" },
    { x: 3, y: 5, c: "#f0854f" },
    { x: 3, y: 6, c: "#f0854f" },
    { x: 3, y: 8, c: "#ffe5ad" },
    { x: 3, y: 9, c: "#ffe5ad" },
    { x: 3, y: 10, c: "#ffe5ad" },
    { x: 4, y: 4, c: "#f0854f" },
    { x: 4, y: 5, c: "#f0854f" },
    { x: 4, y: 6, c: "#000000" },
    { x: 4, y: 7, c: "#000000" },
    { x: 4, y: 8, c: "#ffe5ad" },
    { x: 4, y: 9, c: "#ffe5ad" },
    { x: 4, y: 10, c: "#ffe5ad" },
    { x: 5, y: 5, c: "#f0854f" },
    { x: 5, y: 6, c: "#000000" },
    { x: 5, y: 7, c: "#000000" },
    { x: 5, y: 8, c: "#ffe5ad" },
    { x: 5, y: 9, c: "#ffe5ad" },
    { x: 5, y: 10, c: "#ffe5ad" },
    { x: 6, y: 5, c: "#f0854f" },
    { x: 6, y: 6, c: "#f0854f" },
    { x: 6, y: 7, c: "#f0854f" },
    { x: 6, y: 8, c: "#ffe5ad" },
    { x: 6, y: 9, c: "#ffe5ad" },
    { x: 6, y: 10, c: "#ffe5ad" },
    { x: 7, y: 5, c: "#f0854f" },
    { x: 7, y: 6, c: "#f0854f" },
    { x: 7, y: 7, c: "#f0854f" },
    { x: 7, y: 8, c: "#000000" },
    { x: 7, y: 9, c: "#ffe5ad" },
    { x: 7, y: 10, c: "#ffe5ad" },
    { x: 8, y: 5, c: "#f0854f" },
    { x: 8, y: 6, c: "#f0854f" },
    { x: 8, y: 7, c: "#f0854f" },
    { x: 8, y: 8, c: "#ffe5ad" },
    { x: 8, y: 9, c: "#ffe5ad" },
    { x: 8, y: 10, c: "#ffe5ad" },
    { x: 9, y: 5, c: "#f0854f" },
    { x: 9, y: 6, c: "#000000" },
    { x: 9, y: 7, c: "#000000" },
    { x: 9, y: 8, c: "#ffe5ad" },
    { x: 9, y: 9, c: "#ffe5ad" },
    { x: 9, y: 10, c: "#ffe5ad" },
    { x: 10, y: 4, c: "#f0854f" },
    { x: 10, y: 5, c: "#f0854f" },
    { x: 10, y: 6, c: "#000000" },
    { x: 10, y: 7, c: "#000000" },
    { x: 10, y: 8, c: "#ffe5ad" },
    { x: 10, y: 9, c: "#ffe5ad" },
    { x: 10, y: 10, c: "#ffe5ad" },
    { x: 11, y: 4, c: "#f0854f" },
    { x: 11, y: 5, c: "#f0854f" },
    { x: 11, y: 6, c: "#f0854f" },
    { x: 11, y: 7, c: "#f0854f" },
    { x: 11, y: 8, c: "#ffe5ad" },
    { x: 11, y: 9, c: "#ffe5ad" },
    { x: 11, y: 10, c: "#ffe5ad" },
    { x: 12, y: 4, c: "#f0854f" },
    { x: 12, y: 5, c: "#f0854f" },
    { x: 12, y: 6, c: "#f0854f" },
    { x: 12, y: 7, c: "#f0854f" },
    { x: 12, y: 8, c: "#ffe5ad" },
    { x: 12, y: 9, c: "#ffe5ad" },
    { x: 12, y: 10, c: "#f0854f" },
    { x: 13, y: 6, c: "#f0854f" },
    { x: 13, y: 7, c: "#f0854f" },
    { x: 13, y: 8, c: "#f0854f" },
    { x: 13, y: 9, c: "#f0854f" },
    { x: 3, y: 11, c: "#f0854f" },
    { x: 3, y: 13, c: "#f0854f" },
    { x: 3, y: 14, c: "#f0854f" },
    { x: 3, y: 15, c: "#f0854f" },
    { x: 3, y: 16, c: "#f0854f" },
    { x: 3, y: 17, c: "#f0854f" },
    { x: 4, y: 11, c: "#ffe5ad" },
    { x: 4, y: 12, c: "#ffe5ad" },
    { x: 4, y: 13, c: "#ffe5ad" },
    { x: 4, y: 14, c: "#ffe5ad" },
    { x: 4, y: 15, c: "#ffe5ad" },
    { x: 4, y: 16, c: "#ffe5ad" },
    { x: 4, y: 17, c: "#f0854f" },
    { x: 5, y: 11, c: "#ffe5ad" },
    { x: 5, y: 12, c: "#ffe5ad" },
    { x: 5, y: 13, c: "#ffe5ad" },
    { x: 5, y: 14, c: "#ffe5ad" },
    { x: 5, y: 15, c: "#ffe5ad" },
    { x: 5, y: 16, c: "#ffe5ad" },
    { x: 5, y: 17, c: "#ffe5ad" },
    { x: 6, y: 11, c: "#ffe5ad" },
    { x: 6, y: 12, c: "#ffe5ad" },
    { x: 6, y: 13, c: "#ffe5ad" },
    { x: 6, y: 14, c: "#ffe5ad" },
    { x: 6, y: 15, c: "#ffe5ad" },
    { x: 6, y: 16, c: "#ffe5ad" },
    { x: 6, y: 17, c: "#ffe5ad" },
    { x: 7, y: 11, c: "#ffe5ad" },
    { x: 7, y: 12, c: "#ffe5ad" },
    { x: 7, y: 13, c: "#ffe5ad" },
    { x: 7, y: 14, c: "#ffe5ad" },
    { x: 7, y: 15, c: "#ffe5ad" },
    { x: 7, y: 16, c: "#ffe5ad" },
    { x: 7, y: 17, c: "#ffe5ad" },
    { x: 8, y: 11, c: "#ffe5ad" },
    { x: 8, y: 12, c: "#ffe5ad" },
    { x: 8, y: 13, c: "#ffe5ad" },
    { x: 8, y: 14, c: "#ffe5ad" },
    { x: 8, y: 15, c: "#ffe5ad" },
    { x: 8, y: 16, c: "#ffe5ad" },
    { x: 8, y: 17, c: "#ffe5ad" },
    { x: 9, y: 11, c: "#ffe5ad" },
    { x: 9, y: 12, c: "#ffe5ad" },
    { x: 9, y: 13, c: "#ffe5ad" },
    { x: 9, y: 14, c: "#ffe5ad" },
    { x: 9, y: 15, c: "#ffe5ad" },
    { x: 9, y: 16, c: "#ffe5ad" },
    { x: 9, y: 17, c: "#ffe5ad" },
    { x: 10, y: 11, c: "#ffe5ad" },
    { x: 10, y: 12, c: "#ffe5ad" },
    { x: 10, y: 13, c: "#ffe5ad" },
    { x: 10, y: 14, c: "#ffe5ad" },
    { x: 10, y: 15, c: "#ffe5ad" },
    { x: 10, y: 16, c: "#ffe5ad" },
    { x: 10, y: 17, c: "#f0854f" },
    { x: 11, y: 11, c: "#f0854f" },
    { x: 11, y: 13, c: "#f0854f" },
    { x: 11, y: 14, c: "#f0854f" },
    { x: 11, y: 15, c: "#f0854f" },
    { x: 11, y: 16, c: "#f0854f" },
    { x: 11, y: 17, c: "#f0854f" },
    { x: 12, y: 14, c: "#f0854f" },
    { x: 12, y: 15, c: "#f0854f" },
    { x: 13, y: 14, c: "#f0854f" },
    { x: 13, y: 15, c: "#f0854f" },
    { x: 14, y: 15, c: "#f0854f" },
    { x: 12, y: 16, c: "#f0854f" },
    { x: 12, y: 17, c: "#f0854f" },
    { x: 13, y: 16, c: "#f0854f" },
    { x: 13, y: 17, c: "#f0854f" },
    { x: 14, y: 16, c: "#f0854f" },
    { x: 14, y: 17, c: "#f0854f" },
    { x: 11, y: 18, c: "#f0854f" },
    { x: 10, y: 18, c: "#f0854f" },
    { x: 9, y: 18, c: "#f0854f" },
    { x: 8, y: 18, c: "#f0854f" },
    { x: 7, y: 18, c: "#f0854f" },
    { x: 6, y: 18, c: "#f0854f" },
    { x: 5, y: 18, c: "#f0854f" },
    { x: 4, y: 18, c: "#f0854f" },
    { x: 2, y: 14, c: "#f0854f" },
    { x: 2, y: 15, c: "#f0854f" },
    { x: 2, y: 16, c: "#f0854f" },
    { x: 2, y: 17, c: "#f0854f" },
    { x: 15, y: 16, c: "#f0854f" },
    { x: 15, y: 15, c: "#f0854f" },
    { x: 16, y: 14, c: "#ffe5ad" },
    { x: 16, y: 15, c: "#ffe5ad" },
    { x: 16, y: 13, c: "#ffe5ad" },
    { x: 15, y: 14, c: "#ffe5ad" },
    { x: 6, y: 4, c: "#f0854f" },
    { x: 7, y: 4, c: "#f0854f" },
    { x: 8, y: 4, c: "#f0854f" },
    { x: 9, y: 4, c: "#f0854f" },
    { x: 5, y: 4, c: "#f0854f" },
    { x: 5, y: 3, c: "#f0854f" },
    { x: 6, y: 3, c: "#f0854f" },
    { x: 7, y: 3, c: "#f0854f" },
    { x: 8, y: 3, c: "#f0854f" },
    { x: 9, y: 3, c: "#f0854f" },
    { x: 2, y: 1, c: "#f0854f" },
    { x: 12, y: 1, c: "#f0854f" },
    { x: 3, y: 12, c: "#f0854f" },
    { x: 3, y: 7, c: "#f0854f" },
    { x: 2, y: 13, c: "#f0854f" },
    { x: 1, y: 14, c: "#f0854f" },
    { x: 1, y: 16, c: "#f0854f" },
    { x: 1, y: 15, c: "#f0854f" },
    { x: 11, y: 12, c: "#f0854f" },
    { x: 12, y: 13, c: "#f0854f" },
    { x: 3, y: 1, c: "#000000" },
    { x: 2, y: 0, c: "#000000" },
    { x: 11, y: 1, c: "#000000" },
    { x: 12, y: 0, c: "#000000" },
    { x: 4, y: 2, c: "#000000" },
    { x: 10, y: 2, c: "#000000" },
  ],
  eyes: [
    { x1: 4, y1: 6, x2: 5, y2: 7 },
    { x1: 9, y1: 6, x2: 10, y2: 7 },
  ],
  // Same lo-fi reaction set as the bunny. Rows 0-2 are entirely ear, so the
  // triangles lift cleanly and skew from the head line at row 3.
  lofi: true,
  parts: [
    {
      name: "earL",
      pivot: { x: 3, y: 3 },
      cells: [
        { x: 2, y: 0 },
        { x: 2, y: 1 }, { x: 3, y: 1 },
        { x: 2, y: 2 }, { x: 3, y: 2 }, { x: 4, y: 2 },
      ],
    },
    {
      name: "earR",
      pivot: { x: 11, y: 3 },
      cells: [
        { x: 12, y: 0 },
        { x: 11, y: 1 }, { x: 12, y: 1 },
        { x: 10, y: 2 }, { x: 11, y: 2 }, { x: 12, y: 2 },
      ],
    },
  ],
});

// Hand-tuned in the critter editor (/preview/critter-editor).
const penguin = (): PixelModel => ({
  id: "penguin",
  name: "Pebble the Penguin",
  description: "A slate penguin with a snowy belly and tangerine beak.",
  cells: [
    { x: 2, y: 2, c: "#6c7f93" },
    { x: 2, y: 3, c: "#6c7f93" },
    { x: 2, y: 4, c: "#6c7f93" },
    { x: 2, y: 5, c: "#6c7f93" },
    { x: 2, y: 6, c: "#6c7f93" },
    { x: 2, y: 7, c: "#6c7f93" },
    { x: 2, y: 8, c: "#6c7f93" },
    { x: 2, y: 9, c: "#6c7f93" },
    { x: 2, y: 10, c: "#6c7f93" },
    { x: 2, y: 11, c: "#6c7f93" },
    { x: 2, y: 12, c: "#6c7f93" },
    { x: 2, y: 13, c: "#6c7f93" },
    { x: 2, y: 14, c: "#6c7f93" },
    { x: 2, y: 15, c: "#6c7f93" },
    { x: 2, y: 16, c: "#6c7f93" },
    { x: 3, y: 1, c: "#6c7f93" },
    { x: 3, y: 2, c: "#6c7f93" },
    { x: 3, y: 3, c: "#6c7f93" },
    { x: 3, y: 4, c: "#6c7f93" },
    { x: 3, y: 5, c: "#6c7f93" },
    { x: 3, y: 6, c: "#6c7f93" },
    { x: 3, y: 7, c: "#6c7f93" },
    { x: 3, y: 8, c: "#6c7f93" },
    { x: 3, y: 9, c: "#6c7f93" },
    { x: 3, y: 10, c: "#6c7f93" },
    { x: 3, y: 11, c: "#6c7f93" },
    { x: 3, y: 12, c: "#6c7f93" },
    { x: 3, y: 13, c: "#6c7f93" },
    { x: 3, y: 14, c: "#6c7f93" },
    { x: 3, y: 15, c: "#6c7f93" },
    { x: 3, y: 16, c: "#6c7f93" },
    { x: 3, y: 17, c: "#6c7f93" },
    { x: 4, y: 1, c: "#6c7f93" },
    { x: 4, y: 2, c: "#6c7f93" },
    { x: 4, y: 3, c: "#6c7f93" },
    { x: 4, y: 4, c: "#0a0d0c" },
    { x: 4, y: 5, c: "#0a0d0c" },
    { x: 4, y: 6, c: "#6c7f93" },
    { x: 4, y: 7, c: "#6c7f93" },
    { x: 4, y: 8, c: "#6c7f93" },
    { x: 4, y: 9, c: "#f7f5ec" },
    { x: 4, y: 10, c: "#f7f5ec" },
    { x: 4, y: 11, c: "#f7f5ec" },
    { x: 4, y: 12, c: "#f7f5ec" },
    { x: 4, y: 13, c: "#f7f5ec" },
    { x: 4, y: 14, c: "#f7f5ec" },
    { x: 4, y: 15, c: "#f7f5ec" },
    { x: 4, y: 16, c: "#f7f5ec" },
    { x: 4, y: 17, c: "#6c7f93" },
    { x: 5, y: 1, c: "#6c7f93" },
    { x: 5, y: 2, c: "#6c7f93" },
    { x: 5, y: 3, c: "#6c7f93" },
    { x: 5, y: 4, c: "#0a0d0c" },
    { x: 5, y: 5, c: "#0a0d0c" },
    { x: 5, y: 6, c: "#6c7f93" },
    { x: 5, y: 7, c: "#6c7f93" },
    { x: 5, y: 8, c: "#6c7f93" },
    { x: 5, y: 9, c: "#f7f5ec" },
    { x: 5, y: 10, c: "#f7f5ec" },
    { x: 5, y: 11, c: "#f7f5ec" },
    { x: 5, y: 12, c: "#f7f5ec" },
    { x: 5, y: 13, c: "#f7f5ec" },
    { x: 5, y: 14, c: "#f7f5ec" },
    { x: 5, y: 15, c: "#f7f5ec" },
    { x: 5, y: 16, c: "#f7f5ec" },
    { x: 5, y: 17, c: "#f7f5ec" },
    { x: 6, y: 1, c: "#6c7f93" },
    { x: 6, y: 2, c: "#6c7f93" },
    { x: 6, y: 3, c: "#6c7f93" },
    { x: 6, y: 4, c: "#6c7f93" },
    { x: 6, y: 5, c: "#6c7f93" },
    { x: 6, y: 6, c: "#6c7f93" },
    { x: 6, y: 7, c: "#6c7f93" },
    { x: 6, y: 8, c: "#6c7f93" },
    { x: 6, y: 9, c: "#f7f5ec" },
    { x: 6, y: 10, c: "#f7f5ec" },
    { x: 6, y: 11, c: "#f7f5ec" },
    { x: 6, y: 12, c: "#f7f5ec" },
    { x: 6, y: 13, c: "#f7f5ec" },
    { x: 6, y: 14, c: "#f7f5ec" },
    { x: 6, y: 15, c: "#f7f5ec" },
    { x: 6, y: 16, c: "#f7f5ec" },
    { x: 6, y: 17, c: "#f7f5ec" },
    { x: 7, y: 1, c: "#6c7f93" },
    { x: 7, y: 2, c: "#6c7f93" },
    { x: 7, y: 3, c: "#6c7f93" },
    { x: 7, y: 4, c: "#6c7f93" },
    { x: 7, y: 5, c: "#6c7f93" },
    { x: 7, y: 6, c: "#f0885c" },
    { x: 7, y: 7, c: "#f0885c" },
    { x: 7, y: 8, c: "#6c7f93" },
    { x: 7, y: 9, c: "#f7f5ec" },
    { x: 7, y: 10, c: "#f7f5ec" },
    { x: 7, y: 11, c: "#f7f5ec" },
    { x: 7, y: 12, c: "#f7f5ec" },
    { x: 7, y: 13, c: "#f7f5ec" },
    { x: 7, y: 14, c: "#f7f5ec" },
    { x: 7, y: 15, c: "#f7f5ec" },
    { x: 7, y: 16, c: "#f7f5ec" },
    { x: 7, y: 17, c: "#f7f5ec" },
    { x: 8, y: 1, c: "#6c7f93" },
    { x: 8, y: 2, c: "#6c7f93" },
    { x: 8, y: 3, c: "#6c7f93" },
    { x: 8, y: 4, c: "#6c7f93" },
    { x: 8, y: 5, c: "#6c7f93" },
    { x: 8, y: 6, c: "#f0885c" },
    { x: 8, y: 7, c: "#f0885c" },
    { x: 8, y: 8, c: "#6c7f93" },
    { x: 8, y: 9, c: "#f7f5ec" },
    { x: 8, y: 10, c: "#f7f5ec" },
    { x: 8, y: 11, c: "#f7f5ec" },
    { x: 8, y: 12, c: "#f7f5ec" },
    { x: 8, y: 13, c: "#f7f5ec" },
    { x: 8, y: 14, c: "#f7f5ec" },
    { x: 8, y: 15, c: "#f7f5ec" },
    { x: 8, y: 16, c: "#f7f5ec" },
    { x: 8, y: 17, c: "#f7f5ec" },
    { x: 9, y: 1, c: "#6c7f93" },
    { x: 9, y: 2, c: "#6c7f93" },
    { x: 9, y: 3, c: "#6c7f93" },
    { x: 9, y: 4, c: "#6c7f93" },
    { x: 9, y: 5, c: "#6c7f93" },
    { x: 9, y: 6, c: "#6c7f93" },
    { x: 9, y: 7, c: "#6c7f93" },
    { x: 9, y: 8, c: "#6c7f93" },
    { x: 9, y: 9, c: "#f7f5ec" },
    { x: 9, y: 10, c: "#f7f5ec" },
    { x: 9, y: 11, c: "#f7f5ec" },
    { x: 9, y: 12, c: "#f7f5ec" },
    { x: 9, y: 13, c: "#f7f5ec" },
    { x: 9, y: 14, c: "#f7f5ec" },
    { x: 9, y: 15, c: "#f7f5ec" },
    { x: 9, y: 16, c: "#f7f5ec" },
    { x: 9, y: 17, c: "#f7f5ec" },
    { x: 10, y: 1, c: "#6c7f93" },
    { x: 10, y: 2, c: "#6c7f93" },
    { x: 10, y: 3, c: "#6c7f93" },
    { x: 10, y: 4, c: "#0a0d0c" },
    { x: 10, y: 5, c: "#0a0d0c" },
    { x: 10, y: 6, c: "#6c7f93" },
    { x: 10, y: 7, c: "#6c7f93" },
    { x: 10, y: 8, c: "#6c7f93" },
    { x: 10, y: 9, c: "#f7f5ec" },
    { x: 10, y: 10, c: "#f7f5ec" },
    { x: 10, y: 11, c: "#f7f5ec" },
    { x: 10, y: 12, c: "#f7f5ec" },
    { x: 10, y: 13, c: "#f7f5ec" },
    { x: 10, y: 14, c: "#f7f5ec" },
    { x: 10, y: 15, c: "#f7f5ec" },
    { x: 10, y: 16, c: "#f7f5ec" },
    { x: 10, y: 17, c: "#f7f5ec" },
    { x: 11, y: 1, c: "#6c7f93" },
    { x: 11, y: 2, c: "#6c7f93" },
    { x: 11, y: 3, c: "#6c7f93" },
    { x: 11, y: 4, c: "#0a0d0c" },
    { x: 11, y: 5, c: "#0a0d0c" },
    { x: 11, y: 6, c: "#6c7f93" },
    { x: 11, y: 7, c: "#6c7f93" },
    { x: 11, y: 8, c: "#6c7f93" },
    { x: 11, y: 9, c: "#f7f5ec" },
    { x: 11, y: 10, c: "#f7f5ec" },
    { x: 11, y: 11, c: "#f7f5ec" },
    { x: 11, y: 12, c: "#f7f5ec" },
    { x: 11, y: 13, c: "#f7f5ec" },
    { x: 11, y: 14, c: "#f7f5ec" },
    { x: 11, y: 15, c: "#f7f5ec" },
    { x: 11, y: 16, c: "#f7f5ec" },
    { x: 11, y: 17, c: "#6c7f93" },
    { x: 12, y: 1, c: "#6c7f93" },
    { x: 12, y: 2, c: "#6c7f93" },
    { x: 12, y: 3, c: "#6c7f93" },
    { x: 12, y: 4, c: "#6c7f93" },
    { x: 12, y: 5, c: "#6c7f93" },
    { x: 12, y: 6, c: "#6c7f93" },
    { x: 12, y: 7, c: "#6c7f93" },
    { x: 12, y: 8, c: "#6c7f93" },
    { x: 12, y: 9, c: "#6c7f93" },
    { x: 12, y: 10, c: "#6c7f93" },
    { x: 12, y: 11, c: "#6c7f93" },
    { x: 12, y: 12, c: "#6c7f93" },
    { x: 12, y: 13, c: "#6c7f93" },
    { x: 12, y: 14, c: "#6c7f93" },
    { x: 12, y: 15, c: "#6c7f93" },
    { x: 12, y: 16, c: "#6c7f93" },
    { x: 12, y: 17, c: "#6c7f93" },
    { x: 13, y: 2, c: "#6c7f93" },
    { x: 13, y: 3, c: "#6c7f93" },
    { x: 13, y: 4, c: "#6c7f93" },
    { x: 13, y: 5, c: "#6c7f93" },
    { x: 13, y: 6, c: "#6c7f93" },
    { x: 13, y: 7, c: "#6c7f93" },
    { x: 13, y: 8, c: "#6c7f93" },
    { x: 13, y: 9, c: "#6c7f93" },
    { x: 13, y: 10, c: "#6c7f93" },
    { x: 13, y: 11, c: "#6c7f93" },
    { x: 13, y: 12, c: "#6c7f93" },
    { x: 13, y: 13, c: "#6c7f93" },
    { x: 13, y: 14, c: "#6c7f93" },
    { x: 13, y: 15, c: "#6c7f93" },
    { x: 13, y: 16, c: "#6c7f93" },
    { x: 1, y: 8, c: "#6c7f93" },
    { x: 1, y: 9, c: "#6c7f93" },
    { x: 1, y: 10, c: "#6c7f93" },
    { x: 1, y: 11, c: "#6c7f93" },
    { x: 1, y: 12, c: "#6c7f93" },
    { x: 14, y: 7, c: "#6c7f93" },
    { x: 14, y: 8, c: "#6c7f93" },
    { x: 14, y: 9, c: "#6c7f93" },
    { x: 14, y: 10, c: "#6c7f93" },
    { x: 14, y: 11, c: "#6c7f93" },
    { x: 14, y: 12, c: "#6c7f93" },
    { x: 3, y: 18, c: "#f0885c" },
    { x: 3, y: 19, c: "#f0885c" },
    { x: 4, y: 18, c: "#f0885c" },
    { x: 4, y: 19, c: "#f0885c" },
    { x: 5, y: 18, c: "#f0885c" },
    { x: 5, y: 19, c: "#f0885c" },
    { x: 6, y: 18, c: "#f0885c" },
    { x: 6, y: 19, c: "#f0885c" },
    { x: 9, y: 18, c: "#f0885c" },
    { x: 9, y: 19, c: "#f0885c" },
    { x: 10, y: 18, c: "#f0885c" },
    { x: 10, y: 19, c: "#f0885c" },
    { x: 11, y: 18, c: "#f0885c" },
    { x: 11, y: 19, c: "#f0885c" },
    { x: 12, y: 18, c: "#f0885c" },
    { x: 12, y: 19, c: "#f0885c" },
    { x: 14, y: 13, c: "#6c7f93" },
    { x: 14, y: 14, c: "#6c7f93" },
    { x: 13, y: 17, c: "#6c7f93" },
    { x: 2, y: 17, c: "#6c7f93" },
    { x: 2, y: 18, c: "#6c7f93" },
    { x: 7, y: 18, c: "#6c7f93" },
    { x: 8, y: 18, c: "#6c7f93" },
    { x: 13, y: 18, c: "#6c7f93" },
    { x: 1, y: 13, c: "#6c7f93" },
    { x: 1, y: 14, c: "#6c7f93" },
    { x: 1, y: 15, c: "#6c7f93" },
    { x: 1, y: 7, c: "#6c7f93" },
    { x: 1, y: 16, c: "#6c7f93" },
    { x: 14, y: 16, c: "#6c7f93" },
    { x: 14, y: 15, c: "#6c7f93" },
  ],
  eyes: [
    { x1: 10, y1: 4, x2: 11, y2: 5 },
    { x1: 4, y1: 4, x2: 5, y2: 5 },
  ],
  // Lo-fi body reactions + mood props. No rigged parts: a penguin's flippers
  // hang from the shoulder, and the ear rig skews from the *bottom* edge,
  // which would swing them from the tip instead.
  lofi: true,
});

// Hand-tuned in the critter editor (/preview/critter-editor).
const frog = (): PixelModel => ({
  id: "frog",
  name: "Clover the Frog",
  description: "A mossy frog with periscope eyes and a big friendly grin.",
  cells: [
    { x: 2, y: 0, c: "#8fbf6f" },
    { x: 2, y: 1, c: "#8fbf6f" },
    { x: 2, y: 2, c: "#8fbf6f" },
    { x: 2, y: 3, c: "#8fbf6f" },
    { x: 3, y: 0, c: "#8fbf6f" },
    { x: 3, y: 1, c: "#0a0d0c" },
    { x: 3, y: 2, c: "#0a0d0c" },
    { x: 3, y: 3, c: "#8fbf6f" },
    { x: 4, y: 0, c: "#8fbf6f" },
    { x: 4, y: 1, c: "#0a0d0c" },
    { x: 4, y: 2, c: "#0a0d0c" },
    { x: 4, y: 3, c: "#8fbf6f" },
    { x: 5, y: 0, c: "#8fbf6f" },
    { x: 5, y: 1, c: "#8fbf6f" },
    { x: 5, y: 2, c: "#8fbf6f" },
    { x: 5, y: 3, c: "#8fbf6f" },
    { x: 10, y: 0, c: "#8fbf6f" },
    { x: 10, y: 1, c: "#8fbf6f" },
    { x: 10, y: 2, c: "#8fbf6f" },
    { x: 10, y: 3, c: "#8fbf6f" },
    { x: 11, y: 0, c: "#8fbf6f" },
    { x: 11, y: 1, c: "#0a0d0c" },
    { x: 11, y: 2, c: "#0a0d0c" },
    { x: 11, y: 3, c: "#8fbf6f" },
    { x: 12, y: 0, c: "#8fbf6f" },
    { x: 12, y: 1, c: "#0a0d0c" },
    { x: 12, y: 2, c: "#0a0d0c" },
    { x: 12, y: 3, c: "#8fbf6f" },
    { x: 13, y: 0, c: "#8fbf6f" },
    { x: 13, y: 1, c: "#8fbf6f" },
    { x: 13, y: 2, c: "#8fbf6f" },
    { x: 13, y: 3, c: "#8fbf6f" },
    { x: 1, y: 4, c: "#8fbf6f" },
    { x: 1, y: 5, c: "#8fbf6f" },
    { x: 1, y: 6, c: "#8fbf6f" },
    { x: 1, y: 7, c: "#f2a9c4" },
    { x: 1, y: 8, c: "#8fbf6f" },
    { x: 1, y: 9, c: "#8fbf6f" },
    { x: 1, y: 10, c: "#8fbf6f" },
    { x: 1, y: 11, c: "#8fbf6f" },
    { x: 1, y: 12, c: "#8fbf6f" },
    { x: 1, y: 13, c: "#8fbf6f" },
    { x: 1, y: 14, c: "#8fbf6f" },
    { x: 1, y: 15, c: "#8fbf6f" },
    { x: 2, y: 4, c: "#8fbf6f" },
    { x: 2, y: 5, c: "#8fbf6f" },
    { x: 2, y: 6, c: "#8fbf6f" },
    { x: 2, y: 7, c: "#8fbf6f" },
    { x: 2, y: 8, c: "#8fbf6f" },
    { x: 2, y: 9, c: "#8fbf6f" },
    { x: 2, y: 10, c: "#8fbf6f" },
    { x: 2, y: 11, c: "#8fbf6f" },
    { x: 2, y: 12, c: "#8fbf6f" },
    { x: 2, y: 13, c: "#8fbf6f" },
    { x: 2, y: 14, c: "#8fbf6f" },
    { x: 2, y: 15, c: "#8fbf6f" },
    { x: 3, y: 4, c: "#8fbf6f" },
    { x: 3, y: 5, c: "#41522f" },
    { x: 3, y: 6, c: "#8fbf6f" },
    { x: 3, y: 7, c: "#8fbf6f" },
    { x: 3, y: 8, c: "#8fbf6f" },
    { x: 3, y: 9, c: "#8fbf6f" },
    { x: 3, y: 10, c: "#8fbf6f" },
    { x: 3, y: 11, c: "#8fbf6f" },
    { x: 3, y: 12, c: "#8fbf6f" },
    { x: 3, y: 13, c: "#8fbf6f" },
    { x: 3, y: 14, c: "#8fbf6f" },
    { x: 3, y: 15, c: "#8fbf6f" },
    { x: 4, y: 4, c: "#8fbf6f" },
    { x: 4, y: 5, c: "#8fbf6f" },
    { x: 4, y: 6, c: "#41522f" },
    { x: 4, y: 7, c: "#8fbf6f" },
    { x: 4, y: 8, c: "#8fbf6f" },
    { x: 4, y: 9, c: "#d9ebb8" },
    { x: 4, y: 10, c: "#d9ebb8" },
    { x: 4, y: 11, c: "#d9ebb8" },
    { x: 4, y: 12, c: "#d9ebb8" },
    { x: 4, y: 13, c: "#d9ebb8" },
    { x: 4, y: 14, c: "#d9ebb8" },
    { x: 4, y: 15, c: "#d9ebb8" },
    { x: 5, y: 4, c: "#8fbf6f" },
    { x: 5, y: 5, c: "#8fbf6f" },
    { x: 5, y: 6, c: "#41522f" },
    { x: 5, y: 7, c: "#8fbf6f" },
    { x: 5, y: 8, c: "#8fbf6f" },
    { x: 5, y: 9, c: "#d9ebb8" },
    { x: 5, y: 10, c: "#d9ebb8" },
    { x: 5, y: 11, c: "#d9ebb8" },
    { x: 5, y: 12, c: "#d9ebb8" },
    { x: 5, y: 13, c: "#d9ebb8" },
    { x: 5, y: 14, c: "#d9ebb8" },
    { x: 5, y: 15, c: "#d9ebb8" },
    { x: 6, y: 4, c: "#8fbf6f" },
    { x: 6, y: 5, c: "#8fbf6f" },
    { x: 6, y: 6, c: "#41522f" },
    { x: 6, y: 7, c: "#8fbf6f" },
    { x: 6, y: 8, c: "#8fbf6f" },
    { x: 6, y: 9, c: "#d9ebb8" },
    { x: 6, y: 10, c: "#d9ebb8" },
    { x: 6, y: 11, c: "#d9ebb8" },
    { x: 6, y: 12, c: "#d9ebb8" },
    { x: 6, y: 13, c: "#d9ebb8" },
    { x: 6, y: 14, c: "#d9ebb8" },
    { x: 6, y: 15, c: "#d9ebb8" },
    { x: 7, y: 4, c: "#8fbf6f" },
    { x: 7, y: 5, c: "#8fbf6f" },
    { x: 7, y: 6, c: "#41522f" },
    { x: 7, y: 7, c: "#8fbf6f" },
    { x: 7, y: 8, c: "#8fbf6f" },
    { x: 7, y: 9, c: "#d9ebb8" },
    { x: 7, y: 10, c: "#d9ebb8" },
    { x: 7, y: 11, c: "#d9ebb8" },
    { x: 7, y: 12, c: "#d9ebb8" },
    { x: 7, y: 13, c: "#d9ebb8" },
    { x: 7, y: 14, c: "#d9ebb8" },
    { x: 7, y: 15, c: "#d9ebb8" },
    { x: 8, y: 4, c: "#8fbf6f" },
    { x: 8, y: 5, c: "#8fbf6f" },
    { x: 8, y: 6, c: "#41522f" },
    { x: 8, y: 7, c: "#8fbf6f" },
    { x: 8, y: 8, c: "#8fbf6f" },
    { x: 8, y: 9, c: "#d9ebb8" },
    { x: 8, y: 10, c: "#d9ebb8" },
    { x: 8, y: 11, c: "#d9ebb8" },
    { x: 8, y: 12, c: "#d9ebb8" },
    { x: 8, y: 13, c: "#d9ebb8" },
    { x: 8, y: 14, c: "#d9ebb8" },
    { x: 8, y: 15, c: "#d9ebb8" },
    { x: 9, y: 4, c: "#8fbf6f" },
    { x: 9, y: 5, c: "#8fbf6f" },
    { x: 9, y: 6, c: "#41522f" },
    { x: 9, y: 7, c: "#8fbf6f" },
    { x: 9, y: 8, c: "#8fbf6f" },
    { x: 9, y: 9, c: "#d9ebb8" },
    { x: 9, y: 10, c: "#d9ebb8" },
    { x: 9, y: 11, c: "#d9ebb8" },
    { x: 9, y: 12, c: "#d9ebb8" },
    { x: 9, y: 13, c: "#d9ebb8" },
    { x: 9, y: 14, c: "#d9ebb8" },
    { x: 9, y: 15, c: "#d9ebb8" },
    { x: 10, y: 4, c: "#8fbf6f" },
    { x: 10, y: 5, c: "#8fbf6f" },
    { x: 10, y: 6, c: "#41522f" },
    { x: 10, y: 7, c: "#8fbf6f" },
    { x: 10, y: 8, c: "#8fbf6f" },
    { x: 10, y: 9, c: "#d9ebb8" },
    { x: 10, y: 10, c: "#d9ebb8" },
    { x: 10, y: 11, c: "#d9ebb8" },
    { x: 10, y: 12, c: "#d9ebb8" },
    { x: 10, y: 13, c: "#d9ebb8" },
    { x: 10, y: 14, c: "#d9ebb8" },
    { x: 10, y: 15, c: "#d9ebb8" },
    { x: 11, y: 4, c: "#8fbf6f" },
    { x: 11, y: 5, c: "#8fbf6f" },
    { x: 11, y: 6, c: "#41522f" },
    { x: 11, y: 7, c: "#8fbf6f" },
    { x: 11, y: 8, c: "#8fbf6f" },
    { x: 11, y: 9, c: "#d9ebb8" },
    { x: 11, y: 10, c: "#d9ebb8" },
    { x: 11, y: 11, c: "#d9ebb8" },
    { x: 11, y: 12, c: "#d9ebb8" },
    { x: 11, y: 13, c: "#d9ebb8" },
    { x: 11, y: 14, c: "#d9ebb8" },
    { x: 11, y: 15, c: "#d9ebb8" },
    { x: 12, y: 4, c: "#8fbf6f" },
    { x: 12, y: 5, c: "#41522f" },
    { x: 12, y: 6, c: "#8fbf6f" },
    { x: 12, y: 7, c: "#8fbf6f" },
    { x: 12, y: 8, c: "#8fbf6f" },
    { x: 12, y: 9, c: "#8fbf6f" },
    { x: 12, y: 10, c: "#8fbf6f" },
    { x: 12, y: 11, c: "#8fbf6f" },
    { x: 12, y: 12, c: "#8fbf6f" },
    { x: 12, y: 13, c: "#8fbf6f" },
    { x: 12, y: 14, c: "#8fbf6f" },
    { x: 12, y: 15, c: "#8fbf6f" },
    { x: 13, y: 4, c: "#8fbf6f" },
    { x: 13, y: 5, c: "#8fbf6f" },
    { x: 13, y: 6, c: "#8fbf6f" },
    { x: 13, y: 7, c: "#8fbf6f" },
    { x: 13, y: 8, c: "#8fbf6f" },
    { x: 13, y: 9, c: "#8fbf6f" },
    { x: 13, y: 10, c: "#8fbf6f" },
    { x: 13, y: 11, c: "#8fbf6f" },
    { x: 13, y: 12, c: "#8fbf6f" },
    { x: 13, y: 13, c: "#8fbf6f" },
    { x: 13, y: 14, c: "#8fbf6f" },
    { x: 13, y: 15, c: "#8fbf6f" },
    { x: 14, y: 4, c: "#8fbf6f" },
    { x: 14, y: 5, c: "#8fbf6f" },
    { x: 14, y: 6, c: "#8fbf6f" },
    { x: 14, y: 7, c: "#f2a9c4" },
    { x: 14, y: 8, c: "#8fbf6f" },
    { x: 14, y: 9, c: "#8fbf6f" },
    { x: 14, y: 10, c: "#8fbf6f" },
    { x: 14, y: 11, c: "#8fbf6f" },
    { x: 14, y: 12, c: "#8fbf6f" },
    { x: 14, y: 13, c: "#8fbf6f" },
    { x: 14, y: 14, c: "#8fbf6f" },
    { x: 14, y: 15, c: "#8fbf6f" },
    { x: 1, y: 16, c: "#8fbf6f" },
    { x: 1, y: 17, c: "#8fbf6f" },
    { x: 2, y: 16, c: "#8fbf6f" },
    { x: 2, y: 17, c: "#8fbf6f" },
    { x: 2, y: 18, c: "#8fbf6f" },
    { x: 3, y: 16, c: "#8fbf6f" },
    { x: 3, y: 17, c: "#8fbf6f" },
    { x: 3, y: 18, c: "#8fbf6f" },
    { x: 4, y: 16, c: "#8fbf6f" },
    { x: 4, y: 17, c: "#8fbf6f" },
    { x: 4, y: 18, c: "#8fbf6f" },
    { x: 5, y: 16, c: "#8fbf6f" },
    { x: 5, y: 17, c: "#8fbf6f" },
    { x: 5, y: 18, c: "#8fbf6f" },
    { x: 10, y: 16, c: "#8fbf6f" },
    { x: 10, y: 17, c: "#8fbf6f" },
    { x: 10, y: 18, c: "#8fbf6f" },
    { x: 11, y: 16, c: "#8fbf6f" },
    { x: 11, y: 17, c: "#8fbf6f" },
    { x: 11, y: 18, c: "#8fbf6f" },
    { x: 12, y: 16, c: "#8fbf6f" },
    { x: 12, y: 17, c: "#8fbf6f" },
    { x: 12, y: 18, c: "#8fbf6f" },
    { x: 13, y: 16, c: "#8fbf6f" },
    { x: 13, y: 17, c: "#8fbf6f" },
    { x: 13, y: 18, c: "#8fbf6f" },
    { x: 14, y: 16, c: "#8fbf6f" },
    { x: 14, y: 17, c: "#8fbf6f" },
    { x: 0, y: 11, c: "#8fbf6f" },
    { x: 0, y: 12, c: "#8fbf6f" },
    { x: 0, y: 13, c: "#8fbf6f" },
    { x: 0, y: 14, c: "#8fbf6f" },
    { x: 0, y: 15, c: "#8fbf6f" },
    { x: 15, y: 15, c: "#8fbf6f" },
    { x: 15, y: 14, c: "#8fbf6f" },
    { x: 15, y: 13, c: "#8fbf6f" },
    { x: 15, y: 12, c: "#8fbf6f" },
    { x: 15, y: 11, c: "#8fbf6f" },
    { x: 0, y: 7, c: "#8fbf6f" },
    { x: 0, y: 6, c: "#8fbf6f" },
    { x: 0, y: 8, c: "#8fbf6f" },
    { x: 15, y: 6, c: "#8fbf6f" },
    { x: 15, y: 7, c: "#8fbf6f" },
    { x: 15, y: 5, c: "#8fbf6f" },
    { x: 15, y: 8, c: "#8fbf6f" },
    { x: 0, y: 5, c: "#8fbf6f" },
    { x: 1, y: 18, c: "#8fbf6f" },
    { x: 14, y: 18, c: "#8fbf6f" },
  ],
  eyes: [
    { x1: 3, y1: 1, x2: 4, y2: 2 },
    { x1: 11, y1: 1, x2: 12, y2: 2 },
  ],
  // Lo-fi body reactions + mood props. No rigged parts: the frog's eyes sit
  // inside its two turrets, and only one part can carry the blink group — so
  // skewing the turrets would leave an eye behind.
  lofi: true,
});

export const CRITTERS: PixelModel[] = [bunny(), duck(), cat(), fox(), penguin(), frog()];

export type CritterId = "bunny" | "duck" | "cat" | "fox" | "penguin" | "frog";

export const CRITTER_IDS = CRITTERS.map((c) => c.id) as CritterId[];

export const getCritter = (id: string): PixelModel | undefined =>
  CRITTERS.find((c) => c.id === id);

// Map any stored pet_type — including legacy values from before this
// collection existed — onto one of the six current critters, so old profiles
// keep a sensible companion instead of breaking.
const LEGACY_MAP: Record<string, CritterId> = {
  panda: "cat",
  owl: "duck",
  arctic_fox: "fox",
};

export const resolveCritterId = (input?: string | null): CritterId => {
  if (input && CRITTER_IDS.includes(input as CritterId)) return input as CritterId;
  if (input && LEGACY_MAP[input]) return LEGACY_MAP[input];
  return "fox";
};

/** Short companion name, e.g. "Pip" from "Pip the Fox" — used in copy. */
export const critterNick = (id: string): string =>
  getCritter(resolveCritterId(id))?.name.split(" ")[0] ?? "Buddy";
